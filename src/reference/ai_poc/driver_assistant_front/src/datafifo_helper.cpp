            ...
            if (frame->channel == 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (streamer_file != nullptr)
                            streamer_file->
                                write_video_frame(frame->data, frame->data_len, frame->pts, frame->type == 2);
                        if (streamer_rtsp.is_ready())
                ...
                if (_pending_detections_pts != UINT64_MAX)
                {
                    if (streamer_file)
                    {
                        streamer_file->write_detections(_pending_detections_pts, _pending_pre_detections,
                                                        _pending_detections, _pending_detections_situation);
                    }
                    // Broadcast via WebSocket
                    if (ws_server)
                    {
                        ws_server->broadcast_detections(_pending_detections_pts, _pending_detections_situation,
                                                        _pending_detections);
                    }
